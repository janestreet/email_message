...
module rec Multipart : sig
  ...
  let of_octet_stream ~media_type octet_stream =
    ...
    in
    Debug.run_debug (fun () -> eprintf "Boundary: %s; Part count: %d\n"
      (Boundary.to_string boundary)
      (List.length parts));
    let parts =
      List.map ~f:(Message.of_octet_stream ~parent:(Some media_type)) parts;
    in
    {
  ...
  let remove_raw_content t =
    { t with parts = List.map ~f:Message.remove_raw_content t.parts }
  ;;
end
and Content : sig
  type t =
...
= struct
  ...
  let of_octet_stream ~header ~parent octet_stream =
    let media_type =
      Option.value (Header.Content_type.last header)
        ~default:(Header.Content_type.default ~parent)
    in
    let encoding =
      Option.value (Header.Content_transfer_encoding.last header)
  ...
  let create ?(header=Header.empty) ?content () =
    {
      header = header;
      raw_content = None;
      content = Lazy_m.of_val (Option.value content ~default:(Content.empty ()));
    }
  ;;
  ...
  *)
  ...
  let to_string_monoid t =
    let content_string_monoid =
      ...
      | None -> Content.to_string_monoid (Lazy_m.force t.content)
    in
  ...
  let content t = Content.simple (Lazy_m.force t.content);;
  ...
  let t'_of_t t  =
    { Sexp_aux.
      header = t.header;
      content = Lazy_m.force t.content;
    }
  ;;
  ...
  let sexp_of_t = Fn.compose Sexp_aux.sexp_of_t t'_of_t
  let t_of_sexp = Fn.compose t_of_t' Sexp_aux.t_of_sexp
