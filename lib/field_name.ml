...
module Assoc : Assoc_intf.S = struct
  ...
  let remove_last t name = update_last t name ~f:(Fn.const []);;

  let set t ~name ~body =
    try
      update_last t name ~f:(Fn.const [(name, body)])
    with
    Not_found -> add t ~name ~body
  ;;

  let __UNUSED_VALUE__flatten ?(f=(Fn.const true)) t =
  ...
  let map_to_list t ~f = List.rev_map (to_rev_list t) ~f;;
end

module Assoc_concrete (T : Sexpable.S) = struct
  module type S = sig
    type t = T.t Assoc.t;;
