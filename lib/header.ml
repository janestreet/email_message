...
module Typed (T_unsafe : Stringable.S) :
  Typed_intf.S with type el := T_unsafe.t
= struct

  (* Ensures that the headers are properly folded and unfolded. *)
  let to_field_string = Fn.compose Rfc.RFC2822.fold T_unsafe.to_string
  let of_field_string = Fn.compose T_unsafe.of_string Rfc.RFC2822.unfold
    ...
    match (List.find_map (to_rev_list t)
      ~f:(fun (name', str) ->
        if Field_name.equal name name' then
          match on_error with
          | `Ignore -> Option.try_with (fun () -> of_field_string str)
  ...
  let last t ?(on_error=`Ignore) name =
    Option.try_with (fun () -> last_exn t ~on_error name);;

  let all t =
    List.rev_filter_map (to_rev_list t)
      ~f:(fun (field_name, str) ->
        Option.try_with (fun () -> (field_name, of_field_string str)))
  ;;
  ...
  let all_name t name =
    List.rev_filter_map (to_rev_list t)
      ~f:(fun (name', str) ->
        if Field_name.equal name name' then
          Option.try_with (fun () -> of_field_string str)
        else
          None)
  ;;
...
module Typed_single (T : Stringable.S) (C : Constant with type t := Field_name.t)
...
module Content_type = struct
  ...
  let default ~parent =
    if Option.value_map parent ~f:Media_type.is_digest ~default:false then
      default_digest
    else
      default_default
  ;;
