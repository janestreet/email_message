...
= struct
  ...
  (* TEST_MODULE "Lexer.Quantum" = struct
   *   (\** Checks that the translation tables are consistent *\)
   *   TEST = C.String.foldi base64to ~init:true
   *     ~f:(fun i ok c -> ok && (i = Char.code (base64from.[Char.code c])))
   *
   *   TEST = C.String.length base64to = 64;;
   * end *)
...
module Encode_buffer = struct
  ...
  let wrap_line t =
    t.linepos <-
      ...
      | Some wrap ->
        ...
        begin
          C.Bigbuffer.add_string t.buffer "\n";
          0
        end
        else
          t.linepos + 4
  ...
  let add_char' t c =
    ...
    begin
      ...
      C.Bigbuffer.add_char t.buffer (Quantum.base64to.[a]);
      C.Bigbuffer.add_char t.buffer (Quantum.base64to.[b]);
      C.Bigbuffer.add_char t.buffer (Quantum.base64to.[c]);
      C.Bigbuffer.add_char t.buffer (Quantum.base64to.[d])
    end
    else
    begin
...
message t = parse
  | ""
  {
    match C.Queue.dequeue t.LS.buf with
...
field_unstructured_unfold buffer = parse
  | crlf ((wsp as c) ?) {
      C.Bigbuffer.add_char buffer (C.Option.value c ~default:' ');
      field_unstructured_unfold buffer lexbuf
  }
  | wsp as c    {
      if C.Bigbuffer.length buffer > 0 then C.Bigbuffer.add_char buffer c;
      field_unstructured_unfold buffer lexbuf
  }
  | _ as c          {
...
field_unstructured_fold buffer = parse
  ...
  | _ as c {
      if C.Bigbuffer.length buffer = 0 then
        C.Bigbuffer.add_char buffer ' ';
